...
- **Body** — rounded-rect shell, card cavity sized from `cardCount × cardThickness` plus
  clearances, and a thinner-walled **lip** (neck) at the top that the lid telescopes over. An
  optional **thumb notch** dips into the lip's front edge so you can pinch the card stack out; it
  is capped at the lip height, so the closed lid always hides it.
...
- **Closure** — three styles, selectable in the panel:
  ...
  - **Magnets**: four slim vertical pillars — two per wide face — carry pockets for glued-in disc
    magnets (Ø3×2 by default), which meet face-to-face right at the seam. The pillars keep the
...
- **Body style** — the walls can stay **solid** or get an opening cut through both wide faces to
  show the cards: an arched **window**, a row of vertical **slots**, or a **honeycomb** of hexes.
  One slider scales the opening; solid margins are always kept beside, above and below it, and the
  ring under the shoulder stays intact so the lip loads into full wall.
...
- **Openings**: every cutout profile is self-supporting on a vertical wall — the window's teardrop
  ...
  slots and honeycomb bodies also print without supports.
